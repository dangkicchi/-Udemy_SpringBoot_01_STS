...
class TaskServiceImplUnitTest {
    ...
    @DisplayName("更新対象が存在しない場合、例外が発生することを確認するテスト")
        // テスト名
    void throwNotFoundException_update() {

    	Task task1 = new Task();
    	
        // モッククラスのI/Oをセット
    	when(dao.update(task1)).thenReturn(0);

    	//更新対象が存在しない場合、例外が発生することを検査
    	try {
    		taskServiceImpl.update(task1);
    	} catch (TaskNotFoundException e ) {
    		Assertions.assertEquals(e.getMessage(), "更新するタスクが存在しません");
    	}
    }
    ...
    @DisplayName("削除対象が存在しない場合、例外が発生することを確認するテスト")
        // テスト名
    void throwNotFoundException_deleteById() {
